...
volatile long g_angleAltitude = 359000;

// This stores the angle of the Azimuth (or Right Ascension) axis
// To avoid having to deal with floats this stores the angle*1000
volatile long g_angleAzimuth = 359000;
...
LCDML_addAdvanced (2 , LCDML_0_1,          1 , NULL, "",   mDyn_altitude10_0, 0,  _LCDML_TYPE_dynParam);  // Altitude +/- 10.0
LCDML_addAdvanced (3 , LCDML_0_1,          2 , NULL, "",   mDyn_altitude1_0,  0,  _LCDML_TYPE_dynParam);  // Altitude +/-  1.0
LCDML_addAdvanced (4 , LCDML_0_1,          3 , NULL, "",   mDyn_altitude0_1,  0,  _LCDML_TYPE_dynParam);  // Altitude +/-  0.1
LCDML_addAdvanced (5 , LCDML_0_1,          4 , NULL, "",   mDyn_azimuth10_0,  0,  _LCDML_TYPE_dynParam);   // Azimuth +/- 10.0
LCDML_addAdvanced (6 , LCDML_0_1,          5 , NULL, "",   mDyn_azimuth1_0,   0,  _LCDML_TYPE_dynParam);   // Azimuth +/-  1.0
LCDML_addAdvanced (7 , LCDML_0_1,          6 , NULL, "",   mDyn_azimuth0_1,   0,  _LCDML_TYPE_dynParam);   // Azimuth +/-  0.1
LCDML_add         (8 , LCDML_0_1,          0 , "Back",     mFunc_back);

// Main Menu Axis adjustment and display
LCDML_addAdvanced (9 , LCDML_0,        2 , NULL, "",       mDyn_altitude1_0,  0,  _LCDML_TYPE_dynParam);  // Main menu Altitude/Declination adjustment
LCDML_addAdvanced (10, LCDML_0,        3 , NULL, "",       mDyn_azimuth1_0,   0,  _LCDML_TYPE_dynParam);  // Main menu Azimuth/Right Ascension adjustment

// "Targets" sub-menu
LCDML_add        (11  , LCDML_0,       4 , "Targets",               NULL);
LCDML_add        (12  , LCDML_0_4,         1 , "Stars",             NULL);
LCDML_addAdvanced(13  , LCDML_0_4_1,           2 , NULL, "Albireo", mTargets_select_target, 0, _LCDML_TYPE_default);    //     - Albireo
LCDML_addAdvanced(14  , LCDML_0_4_1,           3 , NULL, "Vega",    mTargets_select_target, 1, _LCDML_TYPE_default);    //     - Vega
LCDML_addAdvanced(15  , LCDML_0_4_1,           4 , NULL, "Arktur",  mTargets_select_target, 2, _LCDML_TYPE_default);    //     - Arktur
LCDML_addAdvanced(16  , LCDML_0_4_1,           5 , NULL, "Polaris",    mTargets_select_target, 3, _LCDML_TYPE_default); //     - Polaris
LCDML_addAdvanced(17  , LCDML_0_4_1,           6 , NULL, "Zero",    mTargets_select_target, 4, _LCDML_TYPE_default);    //     - Zero
LCDML_add        (18  , LCDML_0_4_1,           7 , "Back",          mFunc_back);                   //     - Back to "Targets"
LCDML_add        (19  , LCDML_0_4,         2  , "Button counter",   mFunc_p2);      //     - "Button counter" program
LCDML_add        (20  , LCDML_0_4,         3  , "Back"          ,   mFunc_back);            //     - Back to root

// "Special" sub-menu
LCDML_add        (21  , LCDML_0,       5  , "Special"          , NULL);                        // "Special" sub-menu
LCDML_add        (22  , LCDML_0_5,         1  , "Go to Root"       , mFunc_goToRootMenu);      //     - Back to root
LCDML_add        (23  , LCDML_0_5,         2  , "Jump to Time info", mFunc_jumpTo_timer_info); //     - Open timer info
LCDML_add        (24  , LCDML_0_5,         3  , "Back"             , mFunc_back);              //     - Back to root
LCDML_addAdvanced(25  , LCDML_0,       6 ,COND_hide, "screensaver" , mFunc_screensaver, 0,  _LCDML_TYPE_default); // The "screensaver" menu entry which is hidden by default
...
void storeNewData() {
    if (newData == true) {
        if(receivedChars[0] == 's') {
          ...
          if(receivedChars[1] == 'z') {
            // Update azimuth angle
            g_angleAzimuth = getAngleFromCommand(6, 3);
            if(receivedChars[2] == '-') {
              g_angleAzimuth *= -1L;
            }
            Serial.println("Updated angle to ");
            Serial.println(g_angleAzimuth/1000.);
          } else if(receivedChars[1] == 'l') {
            // Update altitude angle
            g_angleAltitude = getAngleFromCommand(6, 3);
            if(receivedChars[2] == '-') {
              g_angleAltitude *= -1L;
            }
            Serial.println("Updated angle to ");
            Serial.println(g_angleAltitude/1000.);
          }
        }
        LCDML.DISP_update();
        newData = false;
